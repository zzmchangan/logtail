...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "          灾难性回溯: re:(a+)+/re:(.*)+ 等在长文本会卡死, 工具启发式识别并 stderr 提醒。\n"
            ...
            "  agent 一次性收集后按 (时间戳, 序列号) 全局排序; 交互环形缓冲也全局按时间有序(慢源掉队插回);\n"
            "  无时间戳行退化为到达时刻; 形似完整日期但越界(99点/2月30/闰秒:60)当无时间戳, 不炸不吞批。\n"
