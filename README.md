...
- `--match` 复用关键词写法: 裸词=子串, `re:`=正则, 大小写不敏感; 逗号/空格分隔多词 = OR。`--exclude` 命中即剔除。
  ...
  ⚠️ **正则灾难性回溯防护**: 形如 `re:(a+)+`/`re:(.*)+`/`re:(\w+)*`(组内含量词、整体再被量化)的正则在长文本上会让 stdlib `re` 回溯爆炸、卡死进程。工具会**启发式识别并 stderr 提醒**(不拦, 仍执行); 交互 `/k`/`/bl` 则会标到命令回显里。真遇到明显卡顿, 先怀疑这类正则。
...
- `--focus <源名[,源名...]>`: **源聚焦**——只输出指定来源的行(dx/glob 源均有效), **支持逗号分隔多源**(如 `--focus clientgate,login` 一轮同看)。**精确匹配、大小写敏感**; 未知/typo 源名 fail-fast exit 2 并列出可用源名(防静默 0 行假阴性)。
...
- `--correlate <key>=<value>`: **关联键**——跨进程按共享标识对齐: 用 `correlation_keys` 配置(或内置预设 `player`/`scene`/`session`)的正则从每行**抽取** id、**归一化**(去空白/前导零)后比对, 只留匹配行, 全局时间排序。解决"同一 id 在不同进程打印成 `player=123`/`RoleId:123`/`"RoleId":"123"`(微服务 JSON)/`guid:123` 串不起来"的问题——内置预设已兼容 JSON 引号写法。未定义 key 回退字面子串(同 `--trace`)。
...
- **动态黑名单三件套**(agent, 仅本次运行、**不写回 config**——TUI 的 `/save` 才落盘, 语义相反别混):
  ...
  - `--blacklist-add WORD[,WORD]`: 追加临时黑名单项。
  - 注意: 黑名单在**采集阶段**应用(先于 `--match`/`--level`), 被 `--level DEBUG` 救不回——要看 Debug 行必须用 `--blacklist-del/--no-blacklist`。
...
- **退出码**: `0`=成功 (含 0 条命中); `2`=配置/参数/正则错误; 仅当加 `--fail-if-empty` 且命中 0 行时才出现 `1` (脚本断言"没有报错")。
- **每行格式**: `{时间戳} {来源:<12} {正文}`。时间戳优先用日志自带原文 (含括号), 无则回退 `[HH:MM:SS.mmm]`; 来源列宽固定 12, 不足补空格。
- **排序**: agent 是**一次性收集后全局排序** (按 `时间戳, 序列号`); **交互版环形缓冲也全局按时间有序**——批次在环内按 (ts_key, seq) 排序, 慢源掉队的行会插回正确位置, 跨源因果链不再被 ~200ms 错位 (仅掉队批触发整环重排, 平时 in-order 追加零开销)。
- **无时间戳的行**: `ts_key` 退化为**到达时刻**, 时间不可靠, 会落在窗口末尾附近 —— 推断时要留意。**形似完整日期但数值越界**(99点/2月30/闰秒 `:60`)的时间戳也按"无时间戳"处理——不会崩、更不会静默丢掉它后面的行。
...
| `/clear` | `/clr` | 清除所有高亮词 |
| `/remove <词> [<词>...]` | `/rm` | 移除指定高亮词 (可一次多个; 大小写不敏感, 同匹配时 `/rm error` 也能删掉 `ERROR`) |
...
| `/unblacklist <规则> [<规则>...]` | `/ubl` | 移除黑名单 (可一次多个; 大小写不敏感, 同匹配) |
...
- **洪峰不卡界面**: 渲染只折视口那一窗的行 (不再每帧折整条 20000 行缓冲), 且每帧排空上限 20000 行, 高并发灌日志时界面保持可响应。**内存有界**: TUI/monitor(实时模式)的采集队列上限 20 万行, 持续洪峰下满则丢弃最旧行(丢弃计数累计), 防长期 OOM; `--agent` 一次性 dump 不设限(它每轮排空全量, 上限由窗口/hard_cap 决定)。
